...
1) Установите `make`, если он не установлен (нужен для удобного запуска Makefile).
  - На Windows можно использовать Git Bash / WSL либо установить `make` через Chocolatey (`choco install make`) или Scoop (`scoop install make`).
...
- Go 1.20+ (доступен в PATH)
- Docker + Docker Compose
- make (или альтернативы: Git Bash / WSL на Windows)
- (опционально) `psql` — для запуска миграций с хоста
